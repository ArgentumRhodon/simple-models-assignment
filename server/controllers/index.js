...
const getName = async (req, res) => {
  try {
    ...
    */
    ...
    return res
      .status(500)
      .json({ error: 'Something went wrong contacting the database' });
  }
};
...
const createDog = async (req, res) => {
  if (
    ...
    return res
      .status(400)
      .json({ error: 'firstname, lastname, breed, and age are all required' });
  }

  // Create data object from req data
  const dogData = {
    name: `${req.body.firstname} ${req.body.lastname}`,
  ...
  // Create a new instance of a Dog using the Dog model
  const newDog = new Dog(dogData);

  // Try to save the dog to the database
  try {
    await newDog.save();
    return res.status(201).json({
      name: newDog.name,
...
// endpoint for aging a searched dog
const ageDog = async (req, res) => {
  ...
  return res
    .status(201)
    .json({ name: doc.name, breed: doc.breed, age: doc.age });
};
